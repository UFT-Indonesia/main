...
public static class RecordManualLogHandler
{
    ...
    /// current before the response goes out. The punch, the recompute and the outboxed event
    /// commit in one transaction (AutoApplyTransactions, Program.cs) — a failed recompute rolls the
    /// punch back, so the error the client sees is true and a retry cannot leave a duplicate.
    /// </summary>
